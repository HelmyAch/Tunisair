...
def context_data():
    context = {
        'page_name' : '',
        'page_title' : '',
        'system_name' : 'Tunisair Managament System',
        'topbar' : True,
        'footer' : True,
    }
...
def save_reservation(request):
    ...
    if not request.method == 'POST':
        ...
        # Add luggage weight validation
        try:
            luggage_weight = float(request.POST.get('luggage_weight', 0))
            luggage_type = request.POST.get('luggage_type', 'hand_carry')
            
            # Validate weight limits
            if luggage_type == 'hand_carry' and luggage_weight > 7:
                resp['msg'] = "Hand carry cannot exceed 7kg"
            elif luggage_type == 'checked_bag' and luggage_weight > 23:
                resp['msg'] = "Checked baggage cannot exceed 23kg"
            elif luggage_type == 'extra_bag' and luggage_weight > 32:
                resp['msg'] = "Extra baggage cannot exceed 32kg"
            else:
                form = forms.SaveReservation(request.POST)
                if form.is_valid():
                    reservation = form.save()
                    resp['status'] = 'success'
                    resp['msg'] = f"Your Reservation #{reservation.id} has been sent. Our staff will contact you soon."
